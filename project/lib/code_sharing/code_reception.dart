...
class _CodeReceptionState extends State<CodeReception> {
  ...
  Future openDialog() => showPlatformDialog(
      context: context,
      builder: (context) => AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                  onPressed: updateDatabase, child: const Text('Join Group')),
            ],
          ));
            ...
            for (var keys2 in type1.entries) {
              ...
              if (keys2.key.toString().contains(groupId.text)) {
                ...
                if (type1.entries.length == count_2) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content:
                          Text("Adding to group ${snapshot2.value} ....")));
                  userRef.update({groupId.text: true});
                  userRef2.update({userEx: true});
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content:
                          Text("You're now added to ${snapshot2.value}!")));
  ...
  void camScanner() async {
    try {
      var id_ob = (await FlutterBarcodeScanner.scanBarcode(
          "#513BDB", 'Cancel', true, ScanMode.QR));
      groupId.text = id_ob;
      updateDatabase();
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      appBar: PlatformAppBar(
        title: PlatformText("Joining Group Options"),
      ),
      body: Center(
        child: Column(
